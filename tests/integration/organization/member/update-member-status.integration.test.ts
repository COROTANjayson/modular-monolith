...
describe('Organization Member - Update Member Status', () => {
  let app: Express;
  ...
  describe('PATCH /api/v1/organizations/:id/members/:userId/status', () => {
    // ========================================
    // Happy Path Tests
    // ========================================

    it('should successfully update member status as OWNER', async () => {
      // Arrange
      ...
      const response = await request(app)
        .patch(`/api/v1/organizations/${org.id}/members/${targetMember.id}/status`)
    ...
    it('should successfully update member status as ADMIN', async () => {
      // Arrange
    ...
    it('should update from suspended to active', async () => {
      // Arrange
      const { accessToken, user: owner } = await createAuthenticatedUser({ email: 'owner@test.com' });
      const { user: targetMember } = await createAuthenticatedUser({ email: 'member@test.com' });
    ...
    it('should return 403 when MEMBER tries to update status', async () => {
      // Arrange
      const { user: owner } = await createAuthenticatedUser({ email: 'owner@test.com' });
      ...
      // Act
      const response = await request(app)
        .patch(`/api/v1/organizations/${org.id}/members/${member2.id}/status`)
    ...
    it('should return 400 when trying to change owner status', async () => {
      // Arrange
      const { accessToken, user: owner } = await createAuthenticatedUser({ email: 'owner@test.com' });
      ...
      // Act
      const response = await request(app)
        .patch(`/api/v1/organizations/${org.id}/members/${owner.id}/status`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send(updateData)
        .expect(400);
      ...
      // Assert
      expect(response.body).toHaveProperty('success', false);
      expect(response.body).toHaveProperty('message', 'Cannot change status of organization owner');
    });
    ...
    it('should return 404 for non-existent member', async () => {
      ...
      // Act
      const response = await request(app)
        .patch(`/api/v1/organizations/${org.id}/members/${nonExistentUserId}/status`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send(updateData)
        .expect(404);
    ...
    it('should return 401 without authorization header', async () => {
      ...
      // Act
      const response = await request(app)
        .patch(`/api/v1/organizations/${org.id}/members/some-user-id/status`)
        .send(updateData)
        .expect(401);
    ...
    it('should verify status updated in database', async () => {
      // Arrange
      const { accessToken, user: owner } = await createAuthenticatedUser({ email: 'owner@test.com' });
      const { user: targetMember } = await createAuthenticatedUser({ email: 'member@test.com' });
